...
CONF_FORCE_MASTER = "force_master"
    ...
    cv.Schema(
        {
            ...
            cv.Optional(CONF_MCLK_MULTIPLE, default=256): cv.int_range(min=64, max=1024),
            cv.Optional(CONF_FORCE_MASTER, default=False): cv.boolean,
        }
    )
...
async def to_code(config):
    ...
    cg.add(var.set_force_master(config[CONF_FORCE_MASTER]))