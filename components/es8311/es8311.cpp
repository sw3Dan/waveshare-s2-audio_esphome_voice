...
void ES8311::setup() {
  ...
  if (this->force_master_) {
    reg00_po |= 0x40;            // keep MSC=1 (master)
  }
  ES8311_ERROR_FAILED(this->write_byte(ES8311_REG00_RESET, reg00_po));
}
...
//  ESP_LOGCONFIG(TAG, "  MCLK multiple (codec): %" PRIu32 "  (MCLK ~= %" PRIu32 " Hz)", this->mclk_multiple_, this->sample_frequency_ * this->mclk_multiple_);
  ...
  if (this->read_byte(ES8311_REG00_RESET, &reg00)) {
    bool master = (reg00 & 0x40) != 0;  // MSC bit
    ESP_LOGCONFIG(TAG, "  I2S Role: %s", master ? "MASTER (driving BCLK/LRCK)" : "SLAVE");
  }

  // If we’re using external MCLK, show the multiple we expect (codec side)
  // MCLK = sample_rate * mclk_multiple_
  {
    uint32_t mclk = this->sample_frequency_ * this->mclk_multiple_;
    ESP_LOGCONFIG(TAG, "  MCLK multiple (codec): %" PRIu32 "  (MCLK ~= %" PRIu32 " Hz)",
                  (uint32_t) this->mclk_multiple_, mclk);
  }

  // Read out key registers for clocking and format
  uint8_t r01=0, r02=0, r03=0, r04=0, r05=0, r06=0, r07=0, r08=0, r09=0, r0A=0, r31=0, r32=0;
  bool ok = true;
  ok &= this->read_byte(ES8311_REG01_CLK_MANAGER, &r01);
  ok &= this->read_byte(ES8311_REG02_CLK_MANAGER, &r02);
  ok &= this->read_byte(ES8311_REG03_CLK_MANAGER, &r03);
  ok &= this->read_byte(ES8311_REG04_CLK_MANAGER, &r04);
  ok &= this->read_byte(ES8311_REG05_CLK_MANAGER, &r05);
  ok &= this->read_byte(ES8311_REG06_CLK_MANAGER, &r06);
  ok &= this->read_byte(ES8311_REG07_CLK_MANAGER, &r07);
  ok &= this->read_byte(ES8311_REG08_CLK_MANAGER, &r08);
  ok &= this->read_byte(ES8311_REG09_SDPIN, &r09);
  ok &= this->read_byte(ES8311_REG0A_SDPOUT, &r0A);
  ok &= this->read_byte(ES8311_REG31_DAC, &r31);
  ok &= this->read_byte(ES8311_REG32_DAC, &r32);

  if (ok) {
    ESP_LOGCONFIG(TAG, "  REG00=0x%02X REG01=0x%02X REG02=0x%02X REG03=0x%02X",
                  reg00, r01, r02, r03);
    ESP_LOGCONFIG(TAG, "  REG04=0x%02X REG05=0x%02X REG06=0x%02X REG07=0x%02X REG08=0x%02X",
                  r04, r05, r06, r07, r08);
    ESP_LOGCONFIG(TAG, "  REG09(SDPIN)=0x%02X REG0A(SDPOUT)=0x%02X  REG31(DAC mute)=0x%02X REG32(DAC vol)=0x%02X",
                  r09, r0A, r31, r32);
  } else {
...
// Helper: numeric bits from enum
static inline uint8_t es8311_bits_from_resolution(ES8311Resolution r) {
  switch (r) {
    case ES8311_RESOLUTION_16: return 16;
    case ES8311_RESOLUTION_18: return 18;
    case ES8311_RESOLUTION_20: return 20;
    case ES8311_RESOLUTION_24: return 24;
    case ES8311_RESOLUTION_32: return 32;
  }
  return 16;
}
...
*/
  // Register 0x06: **override** BCLK divider analytically for master mode.
  // Target: BCLK = fs * bits_per_slot * channels(=2). ES8311 derives both BCLK and LRCK from MCLK.
  // We compute div = round(MCLK / target_bclk) and program it explicitly.
  {
    uint8_t reg06_cur;
    ES8311_ERROR_CHECK(this->read_byte(ES8311_REG06_CLK_MANAGER, &reg06_cur));

    // Preserve INV_SCLK (bit 5), clear div bits
    uint8_t reg06_new = reg06_cur & 0xE0;
    if (this->sclk_inverted_) {
      reg06_new |= (1u << 5);
    } else {
      reg06_new &= ~(1u << 5);
    ...
    // Calculate divider from actual MCLK used above
    uint32_t mclk_frequency = this->sample_frequency_ * this->mclk_multiple_;
    if (!this->use_mclk_) {
      // not expected for master, but keep behavior consistent
      mclk_frequency = this->sample_frequency_ * (uint32_t) es8311_bits_from_resolution(this->resolution_out_) * 2u;
    }

    const uint8_t bits = es8311_bits_from_resolution(this->resolution_out_);
    const uint32_t channels = 2; // ES8311 DAC outputs stereo frames on I2S
    const uint32_t target_bclk = this->sample_frequency_ * bits * channels; // e.g. 16k * 16 * 2 = 512k

    // Round to nearest integer divider, clamp to valid range.
    uint32_t div = (target_bclk == 0) ? 1 : ( (mclk_frequency + (target_bclk/2)) / target_bclk );
    if (div < 1) div = 1;
    if (div > 31) div = 31; // 5 bits available via ES8311 encoding (<=18 uses div-1 encoding)

    // ES8311 REG06 encoding quirk kept from original driver:
    //   if div < 19 -> program (div-1); else program (div)
    uint8_t reg06_div_field = (div < 19) ? (uint8_t)(div - 1) : (uint8_t)div;
    reg06_new |= (reg06_div_field & 0x1F);

    ES8311_ERROR_CHECK(this->write_byte(ES8311_REG06_CLK_MANAGER, reg06_new));

    // Verbose diag to confirm the math on real hardware
    ESP_LOGI(TAG,
             "MASTER clocking: fs=%" PRIu32 " Hz, bits=%u, ch=2 -> BCLK=%" PRIu32
             " Hz, MCLK=%" PRIu32 " Hz, BCLK_DIV=%" PRIu32 " (REG06=0x%02X)",
             this->sample_frequency_, bits, target_bclk, mclk_frequency, div, reg06_new);
  }
  // Register 0x07
  uint8_t reg07;
...
bool ES8311::configure_format_() {
  ...
  if (this->force_master_) {
    reg00 |= BIT(6);   // MSC=1 → MASTER (drive BCLK/LRCK)
  } else {
    reg00 &= ~BIT(6);  // MSC=0 → SLAVE (current default)
  }
